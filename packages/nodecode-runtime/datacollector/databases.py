...
"""
*sepsisdb* - Fetch data from the SepsisDB database server.

The class ``SepsisDB`` provides access to specific tables in the SMITH_SepsisDB database.

Before using for the first time: save passwords using ``save_passwords.py``

To write your own database driver:
    If the database can be found on the SepsisDB server, you can extend this file with your own implementation.
    Otherwise, create a new file that provides low level access to the data source (like "read_sql_query") and
    implement a class with static methods on top of it.
"""
