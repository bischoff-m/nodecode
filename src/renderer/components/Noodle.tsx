...
export default function Noodle(props: NoodleProps) {
  ...
  const [socketKeyLeft, setSocketKeyLeft] = useState<string | undefined>(
    undefined
  );
  const [socketKeyRight, setSocketKeyRight] = useState<string | undefined>(
    undefined
  );
  ...
  function snapsToSocket(handlePos: Vec2D): Socket | undefined {
    ...
    const socketDistances = allSockets.map((socket) =>
      Math.sqrt(
        (socket.pos.x - handlePos.x) ** 2 + (socket.pos.y - handlePos.y) ** 2
      )
    );
    ...
    <>
      ...
      >
        ...
        <path
          ...
          style={{
            display:
              beganDragging || (socketKeyLeft && socketKeyRight) ? '' : 'none',
          }}
